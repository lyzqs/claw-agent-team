...
新增的两块“时点对比”面板使用多序列 time series + 右侧表格图例 + multi tooltip：

- 当鼠标悬浮到任意时刻时，可同时看到该时点多个进程的 CPU 对比
- 当鼠标悬浮到任意时刻时，可同时看到该时点多个进程的常驻内存对比
- 图例按表格模式展示，便于横向比较当时所有主要进程，而不只局限于单条 TopN 曲线

这已经覆盖题目里要求的“总 cpu，内存，百分比”和“哪个进程占用最多，占了多少”，并补上了“按时间点查看多进程对比”的交互能力。
