...
def build_business_overview() -> dict:
    ...
    dashboard['panels'] = [
        ...
        factory.timeseries(
            ...
            y=10,
            targets=[
                {'expr': f'sum(arena_candidates_total{{{filters}}})', 'legend': '候选数', 'refId': 'A'},
                {'expr': f'sum(arena_trade_tickets_total{{{setup_filters}}})', 'legend': '交易卡片', 'refId': 'B'},
                {'expr': f'sum(arena_executed_trades_total{{{trade_filters}}})', 'legend': '已执行交易', 'refId': 'C'},
            ],
        ),
        factory.timeseries(
            title='市场状态分布',
...
def build_runtime_execution_flow() -> dict:
    ...
    dashboard['panels'] = [
        ...
        factory.timeseries(
            ...
            y=10,
            targets=[
                {'expr': 'arena_runtime_loop_duration_seconds{project=~"$project",job=~"$job",instance=~"$instance",stat="avg"}', 'legend': '{{stage}} 平均', 'refId': 'A'},
...
def build_position_holdings_exits() -> dict:
    ...
    dashboard['panels'] = [
        ...
        factory.stat(title='轮动候选数', expr='sum(arena_rotation_candidates_total{project=~"$project",job=~"$job",instance=~"$instance",market_state=~"$market_state"})', unit='none', x=12, y=0),
        ...
        factory.timeseries(
            ...
            y=10,
            targets=[
                {'expr': 'max(arena_holdings_total{project=~"$project",job=~"$job",instance=~"$instance"})', 'legend': '持仓数', 'refId': 'A'},
                {'expr': 'sum(arena_exit_playbooks_total{project=~"$project",job=~"$job",instance=~"$instance",playbook_type=~"$playbook_type"})', 'legend': '退出剧本', 'refId': 'B'},
            ],
        ),
        factory.bargauge(
            title='退出剧本类型分布',
...
def build_review_validation_iteration() -> dict:
    factory = PanelFactory()
    dashboard = base_dashboard(
        title='AT | Arena | 复盘与迭代',
        uid='at-arena-review-validation-iteration',
        tags=['agent-team-grafana', 'arena', 'review'],
    )
    dashboard['panels'] = [
        factory.stat(title='验证样本数', expr='sum(arena_validation_outcomes_total{project=~"$project",job=~"$job",instance=~"$instance"})', unit='none', x=0, y=0),
        factory.stat(title='正向新闻样本', expr='sum(arena_news_score_distribution{project=~"$project",job=~"$job",instance=~"$instance",score_band="positive"})', unit='none', x=6, y=0),
        factory.stat(title='负向新闻样本', expr='sum(arena_news_score_distribution{project=~"$project",job=~"$job",instance=~"$instance",score_band="negative"})', unit='none', x=12, y=0),
        factory.stat(title='AI 决策次数', expr='sum(arena_runtime_events_total{project=~"$project",job=~"$job",instance=~"$instance",event_type="ai-decision"})', unit='none', x=18, y=0),
        factory.timeseries(
            title='验证结果分布',
            unit='none',
            x=0,
        ...
        factory.timeseries(
            ...
            y=5,
            targets=[
                {'expr': 'sum by (score_band, hard_risk) (arena_news_score_distribution{project=~"$project",job=~"$job",instance=~"$instance"})', 'legend': '{{score_band}} / 风险={{hard_risk}}', 'refId': 'A'},
            ],
        ),
        factory.bargauge(
            title='验证结果排行',
